...
function calculateReadability(text: string) {
  ...
  // Enhanced syllable counting with better accuracy using syllable library
...
router.post('/rewrite-grade-level', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to rewrite text for grade level'
