                        ...
                        <div className="flex flex-wrap gap-1">
                          {issue.replacements.slice(0, 3).map((replacement, repIndex) => (
                            <button 
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                if (onApplySuggestion && issue.offset >= 0) {
                                  onApplySuggestion(issue.offset, issue.length, replacement)
                                }
                              }}
                              className={`px-2 py-0.5 ${
                                issue.offset < 0 
                                  ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 cursor-not-allowed opacity-60' 
                              ...
                              title={issue.offset < 0 ? 'Informational only - location in text could not be determined' : `Click to apply: ${replacement}`}
                            >
